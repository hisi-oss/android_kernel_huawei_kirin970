...
static struct hw_mp3336_private_data_t g_mp3336_pdata;

struct hw_flash_ctrl_t g_mp3336_ctrl;
...
static struct hw_mp3336_flash_level_matrix
	g_mp3336_flash_level[MAX_INDEX_MATRIX] = {
	/*
...
static int g_support_dual_led_type = DUAL_LED_TEMPERATURE;
...
struct ut_test_mp3336 {
	int (*hw_mp3336_set_pin_strobe)(struct hw_flash_ctrl_t *flash_ctrl,
		unsigned int state);
	int (*hw_mp3336_set_pin_torch)(struct hw_flash_ctrl_t *flash_ctrl,
		unsigned int state);
	int (*hw_mp3336_set_pin_reset)(struct hw_flash_ctrl_t *flash_ctrl,
		unsigned int state);
	int (*hw_mp3336_init)(struct hw_flash_ctrl_t *flash_ctrl);
	int (*hw_mp3336_exit)(struct hw_flash_ctrl_t *flash_ctrl);
	int (*hw_mp3336_flash_mode)(struct hw_flash_ctrl_t *flash_ctrl,
		int data);
	int (*hw_mp3336_torch_mode_mmi)(struct hw_flash_ctrl_t *flash_ctrl,
		int data);
	int (*hw_mp3336_set_mode)(struct hw_flash_ctrl_t *flash_ctrl,
		void *data);
	int (*hw_mp3336_on)(struct hw_flash_ctrl_t *flash_ctrl, void *data);
	int (*hw_mp3336_brightness)(struct hw_flash_ctrl_t *flash_ctrl,
		void *data);
	int (*hw_mp3336_off)(struct hw_flash_ctrl_t *flash_ctrl);
	int (*hw_mp3336_get_dt_data)(struct hw_flash_ctrl_t *flash_ctrl);
	ssize_t (*hw_mp3336_dual_leds_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*hw_mp3336_dual_leds_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*hw_mp3336_lightness_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*hw_mp3336_lightness_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*hw_mp3336_flash_lightness_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*hw_mp3336_flash_mask_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	int (*hw_mp3336_register_attribute)(struct hw_flash_ctrl_t *flash_ctrl,
		struct device *dev);
	ssize_t (*hw_mp3336_flash_mask_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	int (*hw_mp3336_match)(struct hw_flash_ctrl_t *flash_ctrl);
	void (*hw_mp3336_torch_brightness_set)(struct led_classdev *cdev,
		enum led_brightness brightness);
	int (*hw_mp3336_param_check)(char *buf, unsigned long *param,
		int num_of_par);
};
...
static int hw_mp3336_param_check(char *buf, unsigned long *param,
	int num_of_par);
static int hw_mp3336_set_pin_strobe(struct hw_flash_ctrl_t *flash_ctrl,
	unsigned int state)
{
...
static int hw_mp3336_set_pin_torch(struct hw_flash_ctrl_t *flash_ctrl,
	unsigned int state)
{
	struct hw_mp3336_private_data_t *pdata = NULL;
	int rc = 0;
...
static int hw_mp3336_set_pin_reset(struct hw_flash_ctrl_t *flash_ctrl,
	unsigned int state)
{
	struct hw_mp3336_private_data_t *pdata = NULL;
	int rc = 0;
...
static int hw_mp3336_exit(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	if (!flash_ctrl->func_tbl || !flash_ctrl->func_tbl->flash_off) {
		cam_err("%s flash_ctrl->func_tbl or flash_ctrl->func_tbl->flash_off is NULL",
			__func__);
		return -1;
	}
	...
	int data)
{
	...
	i2c_func->i2c_write(i2c_client, REG_MODE,
		(reg_val & (~STR_MOD)) | LED2_EN  | LED1_EN | STR_FLASH_MODE);
	...
	int data)
{
	...
	i2c_func->i2c_write(i2c_client, REG_L2_TOR,
		pdata->ecurrent[CURRENT_TORCH_MAX_LEVEL_MMI] - level);
	i2c_func->i2c_read(i2c_client, REG_MODE, &reg_val);
	i2c_func->i2c_write(i2c_client, REG_MODE,
		(reg_val | LED1_EN | LED2_EN) & (~STR_MOD));

	return 0;
}

static int hw_mp3336_set_flash_mode(struct hw_flash_ctrl_t *flash_ctrl,
	struct hw_flash_cfg_data *cdata,
	struct hw_mp3336_flash_level_matrix *matrix, unsigned int current_index)
{
	struct hw_flash_i2c_client *i2c_client = NULL;
	struct hw_flash_i2c_fn_t *i2c_func = NULL;
	...
	if (!flash_ctrl || !cdata || !matrix || !flash_ctrl->flash_i2c_client ||
		...
		cam_err("%s flash_ctrl or cdata or matrix is NULL", __func__);
		return -1;
	}
	...
	if (cdata->mode == FLASH_MODE) {
		...
		mode = STR_FLASH_MODE;
		// reg value = 1.51*index
		tmp_led1 = (current_index - matrix->flash_level_min) *
			151 / 100;
		...
		cam_info("FLASH_MODE regcurrenttorch1 = %d, regcurrenttorch2 = %d",
			regcurrentflash1, regcurrentflash2);
	} else {
		mode = ASIST_MODE; // ASIST_MODE for software control torch
		tmp_led1 = (current_index - matrix->flash_level_min) *
			118 / 100;
		// reg value = 1.18 * index
		...
		cam_info("ASIST_MODE regcurrenttorch1 = %d, regcurrenttorch2 = %d",
			regcurrenttorch1, regcurrenttorch2);
	}
	...
	if (cdata->mode == FLASH_MODE) {
		...
		i2c_func->i2c_write(i2c_client, REG_L1_FL,
			(unsigned char)regcurrentflash1);
		i2c_func->i2c_write(i2c_client, REG_L2_FL,
			(unsigned char)regcurrentflash2);
		cam_info("flash1 = 0x%x, flash2 = 0x%x",
			regcurrentflash1, regcurrentflash2);
	} else {
		i2c_func->i2c_write(i2c_client, REG_L1_TOR,
			(unsigned char)regcurrenttorch1);
		i2c_func->i2c_write(i2c_client, REG_L2_TOR,
			(unsigned char)regcurrenttorch2);
...
static int hw_mp3336_set_mode(struct hw_flash_ctrl_t *flash_ctrl, void *data)
{
	...
	if (current_index >= pdata->flash_led_num) {
		cam_err("%s limit flash to current = %u[%u] mode = %d",
			__func__,
			pdata->flash_led_num - 1,
			current_index, cdata->mode);
		// limit max index
		current_index =
			pdata->flash_led_num > 0 ? pdata->flash_led_num - 1 : 0;
	}

	for (i = 0; i < MAX_INDEX_MATRIX; i++) {
		if (g_mp3336_flash_level[i].flash_level_min <= current_index &&
			g_mp3336_flash_level[i].flash_level_max >= current_index) {
			matrix = &g_mp3336_flash_level[i];
			break;
		}
	}

	if (!matrix) {
		cam_err("%s curret out of range mode = %d curret = %u",
			__func__, cdata->mode, current_index);
		return -1;
	}
	rc = hw_mp3336_set_flash_mode(flash_ctrl, cdata, matrix, current_index);
...
static int hw_mp3336_brightness(struct hw_flash_ctrl_t *flash_ctrl, void *data)
{
	...
	if (cdata->mode == FLASH_MODE) {
		...
		if (rc < 0)
			cam_err("%s hw_mp3336_flash_mode faild rc %d",
				__func__, rc);
	} else {
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = scnprintf(buf, PAGE_SIZE, "mode=%d, data=%d\n",
		g_mp3336_ctrl.state.mode, g_mp3336_ctrl.state.data);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cdata.mode == STANDBY_MODE) {
		rc = hw_mp3336_off(&g_mp3336_ctrl);
		if (rc < 0) {
			cam_err("%s mp3336 flash off error", __func__);
			return rc;
		}
	} else {
		rc = hw_mp3336_on(&g_mp3336_ctrl, &cdata);
		if (rc < 0) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cdata.data == INVALID_LEVEL) // invalid data, turn off
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = scnprintf(buf, PAGE_SIZE, "flash_mask_disabled=%d\n",
		g_mp3336_ctrl.flash_mask_enable);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '0') // char '0' for mask disable
		g_mp3336_ctrl.flash_mask_enable = false;
	else
		g_mp3336_ctrl.flash_mask_enable = true;
	...
	enum led_brightness brightness)
{
	...
	if (led_bright == STANDBY_MODE) {
		...
		// 1~3[TORCH_MODE] 4~6[TORCH_LEFT_MODE] 7~9[TORCH_RIGHT_MODE]
		cdata.mode =
			((brightness - 1) / TORCH_BRIGHT_LEVEL) + TORCH_MODE;
		// level 0,1,2
		cdata.data = ((brightness - 1) % TORCH_BRIGHT_LEVEL);

		cam_info("%s brightness=0x%x, mode=%d, data=%d", __func__,
			brightness, cdata.mode, cdata.data);

		rc = hw_mp3336_brightness(&g_mp3336_ctrl, &cdata);
		if (rc < 0) {
...
static struct device_attribute g_mp3336_lightness =
	__ATTR(lightness, 0664, hw_mp3336_lightness_show,
	hw_mp3336_lightness_store); // wr+wr+r


static struct device_attribute g_mp3336_flash_lightness =
	__ATTR(flash_lightness, 0664, hw_mp3336_lightness_show,
	hw_mp3336_flash_lightness_store); // 664:wr+wr+r

static struct device_attribute g_mp3336_dual_leds =
	__ATTR(dual_leds, 0664, hw_mp3336_dual_leds_show,
	hw_mp3336_dual_leds_store); // wr+wr+r

static struct device_attribute g_mp3336_flash_mask =
	__ATTR(flash_mask, 0664, hw_mp3336_flash_mask_show,
	hw_mp3336_flash_mask_store); // wr+wr+r
	...
	struct device *dev)
{
	...
	rc = device_create_file(dev, &g_mp3336_lightness);
	if (rc < 0) {
...
err_create_flash_lightness_file:
	device_remove_file(dev, &g_mp3336_lightness);
...
static int hw_mp3336_match(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	register_camerafs_attr(&g_mp3336_dual_leds);
	/* add for debug only */
	register_camerafs_attr(&g_mp3336_lightness);
...
static int hw_mp3336_remove(struct i2c_client *client)
{
	...
	g_mp3336_ctrl.func_tbl->flash_exit(&g_mp3336_ctrl);
...
static const struct i2c_device_id g_mp3336_id[] = {
	{ "mp3336", (unsigned long) &g_mp3336_ctrl },
	{}
};
...
struct ut_test_mp3336 g_ut_mp3336 = {
