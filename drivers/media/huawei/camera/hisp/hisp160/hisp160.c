 ...
 * Description:Implement of hisp160
 * Author: yangkai
 * Create: 2014-11-11
 *
 ...
 *********************************************/
void hisp160_set_timestamp(unsigned int *timestamp_high, unsigned int *timestamp_low)
{
	...
	fw_micro_second = (micro_second / MICROSECOND_PER_SECOND +
...
void hisp160_handle_msg(hisp_msg_t *msg)
{
	...
	case REQUEST_RESPONSE:
		hisp160_set_timestamp(&(msg->u.ack_request.timestampH),
			&(msg->u.ack_request.timestampL));
		break;
	case MSG_EVENT_SENT:
		hisp160_set_timestamp(&(msg->u.event_sent.timestampH),
 ...
 *********************************************/
static void hisp160_save_rpmsg_data(const void *data, int len)
{
	int rc = 0;
...
static int hisp160_rpmsg_ept_cb(struct rpmsg_device *rpdev,
...
static int hisp160_unmap_a7isp_addr(void *cfg)
{
	...
	cam_info("func %s: a7 %x, size %x", __FUNCTION__, pcfg->param.moduleAddr,
		pcfg->param.size);
...
static int hisp160_get_a7isp_addr(void *cfg)
{
	...
	ret = hisp_get_sg_table(pcfg->param.sharedFd, &(s_hisp160.pdev->dev),
		&buf, &attach, &table);
	if (ret < 0) {
	...
	unsigned int req_size)
{
	...
	if (req_addr != fmt.iova || req_size > fmt.size) {
		cam_err("%s:req_iova:%#x, req_size:%u", __FUNCTION__,
			req_addr, req_size);
		cam_err("%s:real_iova:%#llx, real_size:%llu", __FUNCTION__,
			fmt.iova, fmt.size);
		ret = -ERANGE;
	}
...
static int hisp160_init_r8isp_memory_pool(void *cfg)
{
	...
	s_hisp160.mem_pool[ipool].security_isp_mode =
		pcfg->param.security_isp_mode;
	...
	cam_info("func %s: r8_iova_pool_base=0x%x", __FUNCTION__,
		s_hisp160.mem_pool[ipool].r8_iova);
...
static int hisp160_deinit_r8isp_memory_pool(void *cfg)
{
	...
	rc = memset_s(&(s_hisp160.mem_pool[ipool]),
		sizeof(struct hisp160_mem_pool),
...
static int hisp160_alloc_r8isp_addr(void *cfg)
{
	...
	pcfg->param.vaddr =
		(void*)(((unsigned char*)s_hisp160.mem_pool[ipool].ap_va) +
		offset);
...
static int hisp160_mem_pool_pre_init(void)
{
	...
	for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++) {
		...
		if (prot < 0) {
			cam_err("%s unkown ipool %d prot 0x%x", __FUNCTION__,
				ipool, prot);
			return -EINVAL;
		}
...
static int hisp160_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		if (!remote_processor_up) {
			...
			cam_notice("%s power on the hisp160", __FUNCTION__);
			rc = hisp160_power_on(i);
		} else {
			cam_warn("%s hisp160 is still on power-on state,"
				"power off it", __FUNCTION__);
	...
	case HISP_CONFIG_POWER_OFF:
		if (remote_processor_up) {
			cam_notice("%s power off the hisp160", __FUNCTION__);
			rc = hisp160_power_off(i);
		}
		break;
	case HISP_CONFIG_GET_MAP_ADDR:
		rc = hisp160_get_a7isp_addr(cfg);
		cam_notice("%s get a7 map address 0x%x ", __FUNCTION__,
	...
	case HISP_CONFIG_PROC_TIMEOUT:
		cam_info("%s message_id.0x%x", __FUNCTION__, pcfg->cfgdata[0]);
...
static int hisp160_power_on(hisp_intf_t *i)
{
	...
	hisi_serv->ept = rpmsg_create_ept(hisi_serv->rpdev,
		hisp160_rpmsg_ept_cb, hisi_serv, chinfo);
	if (!hisi_serv->ept) {
		rc = -ENOMEM;
		goto FAILED_RET;
	}
...
static int hisp160_rpmsg_driver_cb(struct rpmsg_device *rpdev,
	const void *data, int len, void *priv, u32 src)
{
	cam_info("%s enter ... ", __FUNCTION__);
...
static int hisp160_send_rpmsg(hisp_intf_t *i, hisp_msg_t *from_user, size_t len)
{
	int rc = 0;
	hisp160_t *hi = NULL;
	struct rpmsg_hisp160_service *hisi_serv = NULL;
...
static int hisp160_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
	int rc = len;
	hisp160_t *hi = NULL;
	struct rpmsg_hisp160_service *hisi_serv = NULL;
	...
	if (skb_queue_empty(&hisi_serv->queue)) {
		...
		if (wait_event_interruptible_timeout(hisi_serv->readq,
			(!skb_queue_empty
			(&hisi_serv->queue) || hisi_serv->state == RPMSG_FAIL),
			HISP_WAIT_TIMEOUT)) {
...
static void hisp160_update_ddrfreq(unsigned int ddr_bandwidth)
{
	...
	if (!atomic_read(&s_hisp160.opened)) {
		cam_info("%s ,cam is not opened,so u can not set ddr bandwidth ", __FUNCTION__);
		return;
	}
...
static struct platform_driver s_hisp160_driver = {
...
static int __init hisp160_init_module(void)
{
	cam_notice("%s enter ", __FUNCTION__);
	return platform_driver_register(&s_hisp160_driver);
}

static void __exit hisp160_exit_module(void)
{
	cam_notice("%s enter ", __FUNCTION__);
	hisp_unregister(s_hisp160.pdev);
