...
struct mutex gc2375_vce_power_lock;
...
static struct sensor_power_setting gc2375_power_down_setting[] = {
	...
	{
		.seq_type = SENSOR_PWDN,
		.config_val = SENSOR_GPIO_HIGH, //out low
		.sensor_index = SENSOR_INDEX_INVALID,
		.delay = DELAY_1MS,
	},
};
...
atomic_t volatile gc2375vce_powered = ATOMIC_INIT(0);
...
static sensor_t s_gc2375 = {
	...
	.p_atpowercnt = &gc2375vce_powered,
};
...
static int32_t gc2375_platform_probe(struct platform_device* pdev)
{
	...
	mutex_init(&gc2375_vce_power_lock);
	np = pdev->dev.of_node;
	if (NULL == np) {
