...
struct mutex imx498imx550hybird_ravel_power_lock;
        ...
        void  *argp)
{
    ...
    switch(data->cfgtype){
        case SEN_CONFIG_POWER_ON:
            mutex_lock(&imx498imx550hybird_ravel_power_lock);
            if (NULL == si->vtbl->power_up)
            {
                cam_err("%s. si power_up is null", __func__);
                /*lint -e455 -esym(455,*)*/
                mutex_unlock(&imx498imx550hybird_ravel_power_lock);
                /*lint -e455 +esym(455,*)*/
                return -EINVAL;
            }
            if (!power_on_status)
...
s_imx498imx550hybird_driver =
{
    ...
    .driver =
    {
        .name = "huawei,imx498imx550hybird_ravel",
        .owner = THIS_MODULE,
        ...
        struct platform_device* pdev)
{
    ...
    mutex_init(&imx498imx550hybird_ravel_power_lock);
    np = pdev->dev.of_node;
    if (NULL == np) {
...
MODULE_DESCRIPTION("imx498imx550hybird_ravel");
