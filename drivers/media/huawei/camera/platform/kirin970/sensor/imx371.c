...
static bool s_imx371_power_on = false;

struct mutex imx371_power_lock;

char const* imx371_get_name(hwsensor_intf_t* si);
int imx371_config(hwsensor_intf_t* si, void  *argp);
int imx371_power_up(hwsensor_intf_t* si);
int imx371_power_down(hwsensor_intf_t* si);
int imx371_match_id(hwsensor_intf_t* si, void * data);
int imx371_csi_enable(hwsensor_intf_t* si);
int imx371_csi_disable(hwsensor_intf_t* si);
...
struct sensor_power_setting hw_imx371_power_up_setting[] = {
...
sensor_t s_imx371 =
{
    ...
    .power_setting_array = {
            .size = ARRAY_SIZE(hw_imx371_power_up_setting),
            .power_setting = hw_imx371_power_up_setting,
     },
     .p_atpowercnt = &imx371_powered,
};

const struct of_device_id s_imx371_dt_match[] =
{
    {
...
int32_t imx371_platform_probe(struct platform_device* pdev)
{
    int rc = 0;
...
int __init
imx371_init_module(void)
{
    cam_notice("enter %s",__func__);
    return platform_driver_probe(&s_imx371_driver,
            imx371_platform_probe);
}

void __exit
imx371_exit_module(void)
{
    if( NULL != s_sensor)
    {
