...
struct mutex ov16b10hybird_col_power_lock;
        ...
        void  *argp)
{
    ...
    switch(data->cfgtype){
        case SEN_CONFIG_POWER_ON:
            mutex_lock(&ov16b10hybird_col_power_lock);
            if (NULL == si->vtbl->power_up)
            {
                cam_err("%s. si power_up is null", __func__);
                /*lint -e455 -esym(455,*)*/
                mutex_unlock(&ov16b10hybird_col_power_lock);
                /*lint -e455 +esym(455,*)*/
                return -EINVAL;
            }
            if (!power_on_status)
        ...
        struct platform_device* pdev)
{
    ...
    mutex_init(&ov16b10hybird_col_power_lock);
...
MODULE_DESCRIPTION("ov16b10hybird_col");
