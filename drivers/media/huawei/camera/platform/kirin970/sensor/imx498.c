...
static struct sensor_power_setting imx498_power_setting [] = {
...
static int imx498_csi_enable(hwsensor_intf_t* si)
{
    return 0;
}

static int imx498_csi_disable(hwsensor_intf_t* si)
{
    return 0;
}
...
static int imx498_do_hw_reset(hwsensor_intf_t* si, int ctl, int id)
{
        ...
        void  *argp)
{
    ...
    switch(data->cfgtype){
        ...
        case SEN_CONFIG_RESET_HOLD:
            ret = imx498_do_hw_reset(si, CTL_RESET_HOLD, data->mode);
            break;
        case SEN_CONFIG_RESET_RELEASE:
            ret = imx498_do_hw_reset(si, CTL_RESET_RELEASE, data->mode);
            break;
        default:
...
static sensor_t s_imx498 =
{
    ...
    .power_setting_array = {
        .size = ARRAY_SIZE(imx498_power_setting),
        .power_setting = imx498_power_setting,
    },
};
...
static int32_t imx498_platform_remove(struct platform_device* pdev);
static struct platform_driver
s_imx498_driver =
{
    .probe = imx498_platform_probe,
    .remove = imx498_platform_remove,
    .driver =
    {
        .name = "huawei,imx498_cor",
        .owner = THIS_MODULE,
        ...
        struct platform_device* pdev)
{
    ...
    id = of_match_node(s_imx498_dt_match, np);
    if (NULL == id) {
...
static int __init
imx498_init_module(void)
{
    cam_info("enter %s",__func__);
    return platform_driver_probe(&s_imx498_driver,
            imx498_platform_probe);
}

static void __exit
imx498_exit_module(void)
{
