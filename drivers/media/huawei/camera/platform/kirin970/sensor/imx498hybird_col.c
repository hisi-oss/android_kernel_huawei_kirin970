...
struct mutex imx498hybird_col_power_lock;
        ...
        void  *argp)
{
    ...
    switch(data->cfgtype){
        case SEN_CONFIG_POWER_ON:
            mutex_lock(&imx498hybird_col_power_lock);
            if (NULL == si->vtbl->power_up)
            {
                cam_err("%s. si power_up is null", __func__);
                /*lint -e455 -esym(455,*)*/
                mutex_unlock(&imx498hybird_col_power_lock);
                /*lint -e455 +esym(455,*)*/
                return -EINVAL;
            }
            if (!power_on_status)
        ...
        struct platform_device* pdev)
{
    ...
    mutex_init(&imx498hybird_col_power_lock);
    np = pdev->dev.of_node;
    if (NULL == np) {
...
MODULE_DESCRIPTION("imx498hybird_col");
