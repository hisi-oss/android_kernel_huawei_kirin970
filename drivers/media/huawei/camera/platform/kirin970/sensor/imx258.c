...
static bool s_imx258_power_on = false;

struct mutex imx258_power_lock;

char const* imx258_get_name(hwsensor_intf_t* si);
int imx258_config(hwsensor_intf_t* si, void  *argp);
int imx258_power_up(hwsensor_intf_t* si);
int imx258_power_down(hwsensor_intf_t* si);
int imx258_match_id(hwsensor_intf_t* si, void * data);
int imx258_csi_enable(hwsensor_intf_t* si);
...
struct sensor_power_setting hw_imx258_power_up_setting[] = {
...
atomic_t volatile imx258_powered = ATOMIC_INIT(0);
sensor_t s_imx258 =
{
    .intf = { .vtbl = &s_imx258_vtbl, },
    .power_setting_array = {
            .size = ARRAY_SIZE(hw_imx258_power_up_setting),
            .power_setting = hw_imx258_power_up_setting,
     },
     .p_atpowercnt = &imx258_powered,
};

const struct of_device_id s_imx258_dt_match[] =
{
    {
...
int imx258_config(hwsensor_intf_t* si, void  *argp)
{
    ...
    switch (data->cfgtype) {
        case SEN_CONFIG_POWER_ON:
            mutex_lock(&imx258_power_lock);
            if (NULL == si->vtbl->power_up)
            {
                cam_err("%s. si power_up is null", __func__);
                /*lint -e455 -esym(455,*)*/
                mutex_unlock(&imx258_power_lock);
                /*lint -e455 +esym(455,*)*/
                return -EINVAL;
            }
            if (!s_imx258_power_on){
                ret = si->vtbl->power_up(si);
                if (0 == ret) {
                    s_imx258_power_on = true;
                } else {
...
int32_t imx258_platform_probe(struct platform_device* pdev)
{
    int rc = 0;
