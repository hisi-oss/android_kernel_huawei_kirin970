...
static SINT32 vp9hal_check_headmsg(mem_buffer_s *vdh_mem_map, SINT32 pic_msg_slot_phy)
{
	UINT32 *slot = NULL;
	...
	UINT32 *pic_height_mb, UINT32 *pic_width_mb)
{
	...
	// D36
	d32 = hw_mem->sed_top_addr & 0xFFFFFFF0;
	wr_msgword(slot + 36, d32);

	// D37
	d32 = hw_mem->pmv_top_addr & 0xFFFFFFF0;
	wr_msgword(slot + 37, d32);

	// D38
	d32 = hw_mem->rcn_top_addr & 0xFFFFFFF0;
	wr_msgword(slot + 38, d32);

	// D39
	d32 = hw_mem->vp9_prob_tab_addr & 0xFFFFFFF0;
	wr_msgword(slot + 39, d32);

	// D40
	d32 = hw_mem->dblk_top_addr & 0xFFFFFFF0;
	wr_msgword(slot + 40, d32);

	// D41
	d32 = hw_mem->vp9_prob_cnt_addr & 0xFFFFFFF0;
	...
	// D43
	d32 = hw_mem->dblk_left_addr & 0xFFFFFFF0;
	wr_msgword(slot + 43, d32);

	// D63
	d32 = hw_mem->msg_slot_addr[DN_MSG_SLOT_INDEX + 1] & 0xFFFFFFF0;
	wr_msgword(slot + 63, d32);

	return VDMHAL_OK;
}

static SINT32 vp9hal_cfgmsg(omxvdh_reg_cfg_s *vdh_reg_cfg, mem_buffer_s *vdh_mem_map)
{
...
void v9hal_basic_cfg(UINT32 d32, omxvdh_reg_cfg_s *vdh_reg_cfg)
{
	...
	((vp9_basic_cfg1 *)(&d32))->max_slcgrp_num     = DEFAULT_SLCGRP_NUM;
...
static SINT32 vp9hal_cfgreg(omxvdh_reg_cfg_s *vdh_reg_cfg)
{
	...
	((ystaddr_1d *)(&d32))->ystaddr_1d =
		(UINT32)(vdh_reg_cfg->vdh_yst_addr) & 0xFFFFFFF0;
...
#ifdef MSG_POOL_ADDR_CHECK
	ret = vp9hal_cfgmsg(vdh_reg_cfg, vdh_mem_map);
	if (ret !=  VDMHAL_OK) {
		return VDMHAL_ERR;
	}
#endif
