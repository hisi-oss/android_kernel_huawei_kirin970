...
static SINT32 mp2hal_cfg_dnmsg(vdmhal_hwmem_s *hw_mem, mem_buffer_s *vdh_mem_map,
							   UINT32 *pic_height_mb, UINT32 *pic_width_mb)
{
	...
	d32 = 0;
	((mp2_dnmsg_d48 *)(&d32))->pmv_top_addr =
		(hw_mem->pmv_top_addr + 0xF) & 0xFFFFFFF0;
	wr_msgword(msg_base + 48, d32);

	slc_dnmsg_phyaddr =
		(hw_mem->msg_slot_addr[DN_MSG_SLOT_INDEX] & 0xFFFFFFF0) + 64 * 4;

	d32 = 0;
	((mp2_dnmsg_d63 *)(&d32))->first_slc_dnmsg_addr = slc_dnmsg_phyaddr;
...
static SINT32 mp2hal_cfgreg(omxvdh_reg_cfg_s *vdh_reg_cfg)
{
	SINT32 d32 = 0;
...
#endif
{
	...
	if (mp2hal_cfgreg(vdh_reg_cfg) != VDMHAL_OK) {
