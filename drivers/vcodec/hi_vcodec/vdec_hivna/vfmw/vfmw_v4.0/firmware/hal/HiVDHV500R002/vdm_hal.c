...
void vdmhal_enableint(SINT32 vdh_id)
{
	...
	if (g_hw_mem[vdh_id].vdm_reg_vir_addr == NULL) {
		...
		if (p32 == NULL) {
			dprint(PRN_FATAL,
				"vdm register virtual address not mapped, reset failed\n");
			return;
		}
...
static SINT32 vdm_hal_check_rp_cfg_address(omxvdh_reg_cfg_s *vdh_reg_cfg, mem_buffer_s *vdh_mem_map)
{
...
#ifdef MSG_POOL_ADDR_CHECK
	vdmhal_assert_ret((SINT32 *)(vdh_mem_map[VDH_SHAREFD_MESSAGE_POOL].start_vir_addr) != NULL,
		"repair msg virtual address is NULL");

	msg_base = (SINT32 *)(vdh_mem_map[VDH_SHAREFD_MESSAGE_POOL].start_vir_addr
		+ (RP_MSG_SLOT_INDEX * MSG_SLOT_RATIO * USE_MSG_SLOT_SIZE));

	for (payload_buf_num = 0; payload_buf_num < MAX_PAYLOAD_BUF_NUM; payload_buf_num++) {
		d32 = 0;
...
SINT32 vdm_hal_cfg_rp_reg(omxvdh_reg_cfg_s *vdh_reg_cfg, mem_buffer_s *vdh_mem_map)
{
	SINT32 d32 = 0x2000C203;
	if (vdm_hal_check_rp_cfg_address(vdh_reg_cfg, vdh_mem_map) != VDMHAL_OK)
		return VDMHAL_ERR;
	write_vreg(VREG_AVM_ADDR, vdh_reg_cfg->vdh_avm_addr, 0);
...
static void vdm_hal_cfg_mem_ctl_reg(void)
{
...
#ifdef VFMW_HEVC_SUPPORT
	vdmhal_assert_ret(vdh_size >= HW_HEVC_MEM_SIZE,
	"VDMHAL_OpenHAL: vdh_size error");
#else
...
#define HEIF_VDM_MSG_BUF_SIZE \
	(HEIF_VDM_MAX_MSG_NUM * HEIF_MSG_UNIT_SIZE * HEIF_VDM_ONE_MSG_SIZE + \
...
SINT32 vdmhal_imp_open_heif_hal(mem_buffer_s *open_param)
{
	...
	for (i = 0; i < HEIF_VDM_MAX_MSG_NUM; i++)
		g_hw_mem[vdh_id].msg_slot_addr[i] =
			phy_addr + i * HEIF_MSG_UNIT_SIZE * HEIF_VDM_ONE_MSG_SIZE;
...
void vdmhal_imp_reset_vdm(SINT32 vdh_id)
{
	...
	if (vdh_id > (MAX_VDH_NUM - 1)) {
	    dprint(PRN_FATAL, "%s: vdh_id : %d is more than the MAX\n", __func__, vdh_id);
	    return;
	}
	...
	*(volatile UINT32 *)vdm_reset_vir_addr = reg |
		(UINT32)MFDE_RESET_CTRL_MASK;
...
void vdmhal_imp_clearintstate(SINT32 vdh_id)
{
	...
	if (g_hw_mem[vdh_id].vdm_reg_vir_addr == NULL) {
		...
		if (p32 != NULL) {
			...
			dprint(PRN_FATAL,
				" %s %d vdm register virtual address not mapped, reset failed\n",
				__func__, __LINE__);
			return;
		}
	}
...
SINT32 vdmhal_imp_check_cfg_address(omxvdh_reg_cfg_s *vdh_reg_cfg, mem_buffer_s *vdh_mem_map)
{
	hi_u32 index;
	hi_s32 ret;
	hi_u32 vdh_cfg;
	hi_u32 map_msg_addr;
	hi_u32 start_stream_phy_addr;
	hi_u32 end_stream_phy_addr;
	...
	vdmhal_assert_ret(vdh_mem_map != NULL, "vdh_mem_map is NULL");
	vdmhal_assert_ret(vdh_mem_map[VDH_SHAREFD_MESSAGE_POOL].is_mapped
		!= 0, "MESSAGE_POOL is not map");
	vdmhal_assert_ret(vdh_reg_cfg->vdh_stream_buf_num
		<= VDH_STREAM_BUF_CNT, "stream buffer num is out of range");
	...
	for (index = VDH_SHAREFD_STREAM_BUF; index < vdh_stream_buf_num; index++) {
		vdmhal_assert_ret(vdh_mem_map[index].is_mapped
			!= 0, "STREAM_BUF is not map");
	...
	vdh_check_cfg_addr_eq_return(vdh_reg_cfg->vdh_vfg_info_addr,
	...
	for (index = VDH_SHAREFD_STREAM_BUF; index < vdh_stream_buf_num; index++) {
		...
		vdh_cfg = ((stream_basic_addr *)
			(&(vdh_reg_cfg->vdh_stream_base_addr)))->stream_base_addr;
	...
	if (ret != VDMHAL_OK) {
		dprint(PRN_FATAL,
			"%s vdh_stream_base_addr is out of range\n", __func__);
		return ret;
	}
...
SINT32 vdmhal_imp_prepare_dec(omxvdh_reg_cfg_s *vdh_reg_cfg, mem_buffer_s *vdh_mem_map)
{
	...
	case VFMW_HEVC:
#ifdef MSG_POOL_ADDR_CHECK
		return hevchal_startdec(vdh_reg_cfg, vdh_mem_map);
#else
		return hevchal_startdec(vdh_reg_cfg);
#endif
#endif
#ifdef VFMW_MPEG2_SUPPORT
	case VFMW_MPEG2:
...
SINT32 vdmhal_is_vdm_run(SINT32 vdh_id)
{
	...
	read_vreg(VREG_VCTRL_STATE, data_32, vdh_id);

	return ((data_32 == 1) ? 0 : 1);
}
...
SINT32 vdmhal_imp_backupinfo(void)
{
	SINT32 i;
	SINT32 reg_tmp;
	g_vdm_reg_state.int_state_reg = vdmhal_imp_checkreg(INT_STATE_REG, 0);
...

	{
		...
		read_vreg(VREG_SED_END0, g_vdm_reg_state.sed_end_0, 0);
		read_vreg(VREG_DEC_CYCLEPERPIC, g_vdm_reg_state.dec_cycle_per_pic, 0);
		read_vreg(VREG_RD_BDWIDTH_PERPIC,
			g_vdm_reg_state.rd_bdwidth_per_pic, 0);
		read_vreg(VREG_WR_BDWIDTH_PERPIC,
			g_vdm_reg_state.wr_bd_width_per_pic, 0);
...
SINT32 vdmhal_imp_prepare_repair(omxvdh_reg_cfg_s *vdh_reg_cfg, mem_buffer_s *vdh_mem_map)
{
	...
	if (hw_mem->vdm_reg_vir_addr == NULL) {
		...
		if (p32 != NULL) {
			...
			dprint(PRN_FATAL,
				"vdm register virtual address not mapped, VDMHAL_PrepareRepair failed\n");
			return VDMHAL_ERR;
		}
	}
	if (vdh_reg_cfg->repair_time == FIRST_REPAIR) {
		if (vdh_reg_cfg->valid_group_num_0 > 0)
			ret = vdm_hal_cfg_rp_reg(vdh_reg_cfg, vdh_mem_map);
		else
			ret = VDMHAL_ERR;
...
void vdmhal_imp_start_hw_repair(SINT32 vdh_id)
{
	...
	vdmhal_imp_clearintstate(vdh_id);
...
SINT32 vdmhal_hw_dec_proc(omxvdh_reg_cfg_s *vdh_reg_cfg, mem_buffer_s* vdh_mem_map)
{
	...
	if (vdh_reg_cfg->vdh_start_hw_dec_flag == 1) {
		ret = vdmhal_imp_prepare_dec(vdh_reg_cfg, vdh_mem_map);
		if (ret == VDMDRV_OK) {
			vdmhal_imp_start_hw_decode(0);
		} else {
			ret = VDMDRV_ERR;
		...
		if (ret == VDMDRV_OK) {
			vdmhal_imp_start_hw_repair(0);
		} else {
			ret = VDMDRV_ERR;
			dprint(PRN_FATAL,
				"%s prepare repair fail\n", __func__);
		}
	} else {
		ret = VDMDRV_ERR;
