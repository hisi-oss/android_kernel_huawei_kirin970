...
SINT32 scd_drv_reset_scd(void)
{
	...
	UINT32 scen_ident_store_val;

	scen_ident_store_val = rd_scd_reg(REG_SCEN_IDENT);
	...
	*(volatile UINT32 *)scd_reset_reg =
	...
	wr_scd_reg(REG_SCEN_IDENT, scen_ident_store_val);
...
SINT32 scd_drv_check_cfg_address(scd_config_reg_s *sm_ctrl_reg, mem_buffer_s *scd_mem_map)
{
	...
	UINT32 max_mem_map_size = scd_mem_map[SCD_SHAREFD_MESSAGE_POOL].start_phy_addr +
		scd_mem_map[SCD_SHAREFD_MESSAGE_POOL].size;
	UINT32 max_scd_buf_num = (sm_ctrl_reg->scd_output_buf_num +
	...
	for (i = SCD_SHAREFD_OUTPUT_BUF; i < max_scd_buf_num; i++) {
		...
		end_output_phy_addr   = scd_mem_map[i].start_phy_addr +
			scd_mem_map[i].size;
		ret = SCDDRV_OK;
...
SINT32 scd_drv_write_reg(scd_config_reg_s *sm_ctrl_reg, mem_buffer_s *scd_mem_map)
{
	...
	// LIST_ADDRESS
	wr_scd_reg(REG_LIST_ADDRESS, (unsigned int)sm_ctrl_reg->down_msg_phy_addr);

	// UP_ADDRESS
	wr_scd_reg(REG_UP_ADDRESS, (UINT32) sm_ctrl_reg->up_msg_phy_addr);

	// UP_LEN
	wr_scd_reg(REG_UP_LEN, (UINT32) sm_ctrl_reg->up_len);

	// BUFFER_FIRST
	wr_scd_reg(REG_BUFFER_FIRST, (UINT32) sm_ctrl_reg->buffer_first);

	// BUFFER_LAST
	wr_scd_reg(REG_BUFFER_LAST, (UINT32) sm_ctrl_reg->buffer_last);

	// BUFFER_INI
	wr_scd_reg(REG_BUFFER_INI, (UINT32) sm_ctrl_reg->buffer_ini);

	// SCD_PROTOCOL
#ifdef ENV_SOS_KERNEL
	wr_scd_reg(REG_SCD_PROTOCOL, (UINT32)((sm_ctrl_reg->scd_lowdly_enable << SCD_LOWDLY_ENABLE_OFFSET) |
	    (1 << SCD_DRV_REG_OFFSET) | ((sm_ctrl_reg->slice_check_flag << SLICE_CHECK_FLAG_OFFSET) & SLICE_CHECK_FLAG_OPR) |
...
void scd_drv_get_reg_state(scd_state_reg_s *scd_state_reg)
{
	...
	ret = memcpy_s(scd_state_reg, sizeof(*scd_state_reg),
