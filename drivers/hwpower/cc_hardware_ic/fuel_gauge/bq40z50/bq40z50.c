...
static bool bq40z50_is_abnormal_low_capacity(s16 cur, u16 vol)
{
	...
	ret = bq40z50_read_word(BQ40Z50_REG_CELL1_VOLT, &cell1_vol);
	if (!ret) {
	...
	ret = bq40z50_read_word(BQ40Z50_REG_CELL2_VOLT, &cell2_vol);
	if (!ret) {
	...
	ret = bq40z50_read_word(BQ40Z50_REG_CELL3_VOLT, &cell3_vol);
	if (!ret) {
...
static int bq40z50_get_battery_soc(void)
{
	...
	if (bq40z50_is_abnormal_low_capacity(di->cache.curr, di->cache.vol)) {
...
{
	return THOUSAND_UNIT * bq40z50_get_battery_vol();
}
