...
static int qi_protocol_send_lightstrap_control_msg(unsigned int ic_type, u8 *msg, int len)
{
	...
	if (len != QI_PARA_LIGHTSTRAP_CTRL_MSG_LEN) {
		hwlog_err("para error %d!=%d\n", len, QI_PARA_LIGHTSTRAP_CTRL_MSG_LEN);
		return -EINVAL;
	}

	if (qi_protocol_auto_send_fsk_msg(ic_type, QI_CMD_LIGHTSTRAP_CTRL, msg, len, retry))
		return -ECOMM;
...
static struct wireless_protocol_ops wireless_protocol_qi_ops = {
	...
	.send_lightstrap_ctrl_msg = qi_protocol_send_lightstrap_control_msg,
