...
static char g_socid[EFUSE_SOCID_LENGTH_BYTES * 2 + 1];
static u32 g_socid_inited;
/*
...
static int __init parse_socid_from_cmdline(char *src)
{
	u32 len;

	if (!src) {
		pr_err("%s:socid src is null.\n", __func__);
		return 0;
	}
	len = strlen(src);
	if (len != sizeof(g_socid) - 1) {
		pr_err("error, len is 0x%x\n", len);
		return -EFAULT;
	}

	memmove((void *)g_socid, (void*)src, len);
	g_socid[len] = '\0';
...
{
	high = isdigit(high) ? high - '0' : (tolower(high) - 'a' + 10);
	low = isdigit(low) ? low - '0' : (tolower(low) - 'a' + 10);

	return (high << 4) | low;
}

static s32 get_socid_value(u8 *buf, u32 size)
{
	int i;
	...
	if (!g_socid_inited) {
		pr_err("%s: socid is not ready now.\n", __func__);
		return -ENODEV;
	}

	for (i = 0; i < size; i++)
		buf[i] = to_char(g_socid[2 * i], g_socid[ 2 * i + 1]);
...
{
	return hisi_efuse_read_value(buf, buf_size, func_id);
}
