...
#define FIX_DSS_RATE 480000000
static struct clk *dss_clk = NULL;
static struct clk *ppll2_clk = NULL;
extern u32 ppll2_en_vote_vol;
extern u32 ppll2_retry_flag;
#endif
...
static const struct pll_clock pll_clks[] = {
	...
	{ CLK_GATE_PPLL2_B, "clk_ap_ppll2_b", "clk_ppll2_b", 0x9,
		{0x96C, 10}, {0x970, 10}, {0x974, 10}, {0x864, 26}, 2, 0, "clk_ap_ppll2_b" },
...
static const struct pll_clock hsdtcrg_pll_clks[] = {
	{ CLK_GATE_PCIE0PLL, "clk_ap_pcie0pll", "clk_ppll_pcie", 0xD,
		{0x188, 0}, {0, 0}, {0x188, 1}, {0x204, 0}, 10, 1, "clk_ap_pcie0pll" },
};
...
static const struct pll_clock hsdt1crg_pll_clks[] = {
	{ CLK_GATE_PCIE1PLL, "clk_ap_pcie1pll", "clk_ppll_pcie1", 0xE,
		{0x198, 0}, {0, 0}, {0x198, 1}, {0x208, 0}, 12, 1, "clk_ap_pcie1pll" },
};
...
static const struct pmu_clock pmu_clks[] = {
	...
	{ CLK_NFC, "clk_nfc", "clk_sys_ini", 0x42, 0, INVALID_HWSPINLOCK_ID, 0, "clk_nfc" },
};
...
#ifdef CONFIG_BALT_IP_PLATFORM
	ppll2_clk = clk_get(NULL, "clk_ap_ppll2");
	if (!IS_ERR_OR_NULL(ppll2_clk) && ppll2_clk->core)
		pr_info("%s ppll2 enable count = %u\n", __func__, ppll2_clk->core->enable_count);
	else
		pr_info("%s get ppll2_clk fail\n", __func__);
#endif
}

CLK_OF_DECLARE_DRIVER(clk_peri_crgctrl,
	"hisilicon,clk-cs-extreme-crgctrl", clk_crgctrl_init);
...
CLK_OF_DECLARE_DRIVER(clk_hsdtcrg,
	"hisilicon,clk-cs-extreme-hsdt-crg", clk_hsdt_init);
...
CLK_OF_DECLARE_DRIVER(clk_hsdt1crg,
	"hisilicon,clk-cs-extreme-hsdt1-crg", clk_hsdt1_init);
...
CLK_OF_DECLARE_DRIVER(clk_sctrl,
	"hisilicon,clk-cs-extreme-sctrl", clk_sctrl_init);
...
CLK_OF_DECLARE_DRIVER(clk_media1crg,
	"hisilicon,clk-cs-extreme-media1-crg", clk_media1_init);

static void clk_media2_init(struct device_node *np)
{
	struct clock_data *clk_data = NULL;
	int nr = ARRAY_SIZE(media2crg_scgt_clks) +
...
CLK_OF_DECLARE_DRIVER(clk_media2crg,
	"hisilicon,clk-cs-extreme-media2-crg", clk_media2_init);
...
CLK_OF_DECLARE_DRIVER(clk_iomcu,
	"hisilicon,clk-cs-extreme-iomcu-crg", clk_iomcu_init);
...
CLK_OF_DECLARE_DRIVER(clk_xfreq,
	"hisilicon,clk-cs-extreme-xfreq", clk_xfreq_init);
...
CLK_OF_DECLARE_DRIVER(clk_pmu,
	"hisilicon,clk-cs-extreme-pmu", clk_pmu_init);


static void clk_fast_dvfs_init(struct device_node *np)
{
	struct clock_data *clk_data = NULL;
	int nr = ARRAY_SIZE(fast_dvfs_clks);
...
CLK_OF_DECLARE_DRIVER(clk_fast_dvfs,
	"hisilicon,clk-cs-extreme-fast-dvfs", clk_fast_dvfs_init);
...
#ifdef CONFIG_BALT_IP_PLATFORM
	dss_clk = clk_get(NULL, "clk_edc0");
	if (!IS_ERR_OR_NULL(dss_clk) && dss_clk->core)
		pr_info("%s clk_edc0 enable count = %u\n", __func__, dss_clk->core->enable_count);
	else
		pr_info("%s get clk_edc0 fail\n", __func__);
#endif
}
CLK_OF_DECLARE_DRIVER(clk_dvfs,
	"hisilicon,clk-cs-extreme-dvfs", clk_dvfs_init);

#ifdef CONFIG_BALT_IP_PLATFORM
void print_clk_mntn_info(void)
{
		pr_err("%s ppll2_en_vote_vol = 0x%x, ppll2_retry_flag = %u\n", __func__, ppll2_en_vote_vol, ppll2_retry_flag);
		if (!IS_ERR_OR_NULL(ppll2_clk) && ppll2_clk->core)
			pr_err("%s ppll2 enable count = %u\n", __func__, ppll2_clk->core->enable_count);
		if (!IS_ERR_OR_NULL(dss_clk) && dss_clk->core)
			pr_err("%s clk_edc0 enable count = %u\n", __func__, dss_clk->core->enable_count);
}
#endif