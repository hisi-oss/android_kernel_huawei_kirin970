...
#ifdef WIN32
char *g_hcc_bus_type_str[HCC_BUS_BUTT] = { "sdio", "pcie", "usb" };
#else
...
int32_t hi110x_hcc_dev_bindcpu(int32_t is_bind, oal_bool_enum_uint8 is_userctl,
                               uint8_t irq_cpu, uint8_t thread_cmd, uint8_t is_lowest_load)
{
    ...
    return hcc_bus_bindcpu(pst_bus, HCC_DIR_COUNT, *(int32_t *)&bindcpu_cfg, is_lowest_load);
}
...
int32_t hcc_dev_bindcpu(uint32_t dev_id, int32_t is_bind)
{
    ...
    pst_bus = pst_bus_dev->cur_bus;
    if (hcc_bus_auto_bindcpu_is_support(dev_id)) {
        return hcc_bus_bindcpu(pst_bus, HCC_DIR_COUNT, is_bind, 0);
    }
    return -OAL_EIO;
}
    ...
    if (oal_time_after(current_time, (pst_bus_dev->bus_pps_start_time + 1 * HZ))) {
        ...
        if (hcc_bus_auto_bindcpu_is_support(dev_id)) {
            ...
            if (pkt_pps_total >= g_hcc_bus_auto_bindcpu_limit) {
                hcc_bus_bindcpu(pst_bus, HCC_DIR_COUNT, 1, 0);
            } else if (pkt_pps_total <= g_hcc_bus_auto_bindcpu_limit / 2) { /* limit除以2为不绑定的切换门限 */
                hcc_bus_bindcpu(pst_bus, HCC_DIR_COUNT, 0, 0);
            }
#endif
        }

        oal_print_hi11xx_log(HI11XX_LOG_DBG, "tx: %lu, rx: %lu, runtime:%u", pkt_pps_tx, pkt_pps_rx, run_time);
...
OAL_STATIC void hcc_bus_hcctransfer_bindcpu(struct hcc_handler *hcc,
    int32_t is_bind, oal_bool_enum_uint8 is_userctl, uint8_t thread_cpu, cpu_cap_mask *cpu_mask)
{
    ...
    if (is_userctl == OAL_TRUE) {
        ...
        cpumask_clear_cpu(OAL_BUS_HPCPU_NUM, &cpu_mask->fast_cpus);
        oal_print_hi11xx_log(HI11XX_LOG_DBG, "hcc thread hisi rt opt cpu");
        if (is_bind) {
            set_cpus_allowed_ptr(hcc->hcc_transer_info.hcc_transfer_thread, &cpu_mask->fast_cpus);
        } else {
            set_cpus_allowed_ptr(hcc->hcc_transer_info.hcc_transfer_thread, &cpu_mask->slow_cpus);
        }
    }
}
    ...
    int32_t is_bind, oal_bool_enum_uint8 is_userctl, uint8_t thread_cpu, cpu_cap_mask *cpu_mask)
{
    ...
    if (is_userctl == OAL_TRUE) {
        ...
        if (is_bind) {
            set_cpus_allowed_ptr(hi_bus->pst_rx_tsk, &cpu_mask->fast_cpus);
        } else {
            set_cpus_allowed_ptr(hi_bus->pst_rx_tsk, &cpu_mask->slow_cpus);
        }
    }
}
...
#ifdef CONFIG_ARCH_PLATFORM
        get_fast_cpus(&tmp);
        get_slow_cpus(&cpu_mask->slow_cpus);
#else
        hisi_get_fast_cpus(&tmp);
        hisi_get_slow_cpus(&cpu_mask->slow_cpus);
#endif
        cpumask_clear_cpu(OAL_BUS_HPCPU_NUM, &tmp);
        cpu = find_lowest_load_cpu(&tmp);
        cpu_mask->fast_cpus = *(struct cpumask *)cpumask_of(cpu);
    } else {
#ifdef CONFIG_ARCH_PLATFORM
        get_fast_cpus(&cpu_mask->fast_cpus);
        get_slow_cpus(&cpu_mask->slow_cpus);
#else
        hisi_get_fast_cpus(&cpu_mask->fast_cpus);
        hisi_get_slow_cpus(&cpu_mask->slow_cpus);
#endif
    }
}
#endif
int32_t hcc_bus_bindcpu(hcc_bus *hi_bus, uint32_t chan, int32_t is_bind, uint8_t is_lowest_load)
{
    oal_pcie_bindcpu_cfg bindcpu_cfg = *(oal_pcie_bindcpu_cfg *)&is_bind;
    struct hcc_handler *hcc = NULL;
#ifdef _PRE_HISI_BINDCPU
    cpu_cap_mask cpu_mask;
#endif
...
#ifdef _PRE_HISI_BINDCPU
    hcc_get_cpus(&cpu_mask, is_lowest_load);
#endif
    if (oal_unlikely(hcc != NULL)) {
        if (hcc->hcc_transer_info.hcc_transfer_thread) {
            ...
            hcc_bus_hcctransfer_bindcpu(hcc, bindcpu_cfg.is_bind,
                bindcpu_cfg.is_userctl, bindcpu_cfg.thread_cmd, &cpu_mask);
#endif
        }
    }
#ifdef _PRE_HISI_BINDCPU
    ...
    if (hi_bus->pst_rx_tsk != NULL) {
        /* 绑定线程pst_rx_tsk */
#ifdef _PRE_HISI_BINDCPU
        hcc_bus_rx_tsk_bindcpu(hi_bus, bindcpu_cfg.is_bind, bindcpu_cfg.is_userctl, bindcpu_cfg.thread_cmd, &cpu_mask);
#endif
    }
...
void hcc_bus_exception_submit(hcc_bus *hi_bus, int32_t excep_type)
{
    ...
    if (oal_work_is_busy(&hi_bus->bus_excp_worker)) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "excep %d, exception %d is working", excep_type, hi_bus->bus_excp_type);
...
OAL_STATIC int32_t hcc_bus_resource_alloc(hcc_bus *pst_bus)
{
    ...
    hcc_bus_message_register(pst_bus, D2H_MSG_DEVICE_PANIC,
                             hcc_bus_device_panic_callback, pst_bus);

    hcc_bus_message_register(pst_bus, D2H_MSG_WLAN_READY,
                             hcc_bus_device_init_ready, pst_bus);
