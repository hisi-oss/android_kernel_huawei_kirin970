...
uint32_t hmac_hid2d_scan_chan_start(mac_vap_stru *pst_mac_vap, mac_device_stru *pst_mac_device)
{
    ...
    ret = hmac_hid2d_scan_chan_once(pst_mac_vap, uc_scan_chan_idx, HMAC_HID2D_APK_SCAN);
    if (ret != OAL_SUCC) {
...
uint32_t hmac_hid2d_scan_chan_once(mac_vap_stru *pst_mac_vap, uint8_t chan_index, uint8_t scan_mode)
{
    ...
    if (oal_value_in_valid_range(pst_mac_vap->en_vap_state, MAC_VAP_STATE_STA_JOIN_COMP, MAC_VAP_STATE_STA_WAIT_ASOC) ||
        pst_mac_vap->en_vap_state == MAC_VAP_STATE_ROAMING) {
        oam_warning_log0(pst_mac_vap->uc_vap_id, 0, "{hmac_hid2d_scan_chan_once::vap is connecting, can not scan!}");
        return OAL_FAIL;
    }

    pst_hmac_hid2d_acs_info = &g_st_hmac_hid2d_acs_info;
    /* 如果是apk触发的扫描 */
    if (scan_mode == HMAC_HID2D_APK_SCAN) {
        scan_channel.en_band = WLAN_BAND_5G;
        pst_candidate_list = g_aus_channel_candidate_list_5g;
    } else if (scan_mode == HMAC_HID2D_ACS_SCAN) {
    ...
    st_h2d_scan_params.st_scan_params.us_scan_time = (scan_mode == HMAC_HID2D_APK_SCAN) ?
...
uint32_t hmac_hid2d_timeout_handle(void *p_arg)
{
    ...
    if ((pst_hmac_hid2d_acs_info->update_num < HMAC_HID2D_SCAN_TIMES_PER_CHAN_FOR_APK) ||
        ...
        /* 再下发一次扫描，并开启定时器 */
        hmac_hid2d_scan_chan_once(pst_p2p_vap, uc_scan_chan_idx, HMAC_HID2D_APK_SCAN);
        frw_timer_create_timer_m(&pst_hmac_hid2d_acs_info->st_scan_chan_timer,
            hmac_hid2d_timeout_handle,
            HMAC_HID2D_SCAN_TIMER_CYCLE_MS,
            pst_mac_device,
    ...
    hmac_hid2d_auto_channel_switch_stru *hmac_hid2d_acs_info)
{
    ...
    oam_warning_log3(0, 0, "HiD2D ACS: report_chan_num=[%d], stats_cnt=[%d], free_power_cnt=[%d]!",
        chan_result->uc_channel_number, chan_result->uc_stats_cnt, chan_result->uc_free_power_cnt);
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    oal_cfg80211_hid2d_acs_report(hmac_vap->pst_net_device, GFP_KERNEL,
        (uint8_t *)(&chan_stats_report), sizeof(chan_stats_report));
#endif
