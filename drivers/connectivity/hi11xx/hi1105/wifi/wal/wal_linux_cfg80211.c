...
void wal_check_cookie_timeout(cookie_arry_stru *cookie_array, uint8_t *cookie_bitmap, uint32_t current_time)
{
    uint8_t uc_loops = 0;
    ...
    for (uc_loops = 0; uc_loops < WAL_COOKIE_ARRAY_SIZE; uc_loops++) {
        pst_tmp_cookie = &cookie_array[uc_loops];
        if (oal_time_after32(OAL_TIME_JIFFY,
            ...
            /* 清除占用的cookie bitmap位 */
            oal_bit_clear_bit_one_byte(cookie_bitmap, uc_loops);
        }
    }
}
