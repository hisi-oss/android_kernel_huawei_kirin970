 ...
 */
    if (pst_rx_ctrl->st_rx_info.bit_amsdu_enable == OAL_FALSE) {
        ...
        if (hmac_11i_ether_type_filter(pst_vap, &pst_hmac_user->st_user_base_info,
            ...
            oam_report_eth_frame(auc_da, (oal_uint8 *)pst_ether_hdr, (oal_uint16)oal_netbuf_len(pst_netbuf),
                                 OAM_OTA_FRAME_DIRECTION_TYPE_RX);
...
OAL_STATIC oal_void hmac_parse_arp_packet(oal_void *pst_eth, oal_uint32 buf_len)
{
    ...
    iph = (struct iphdr *)((mac_ether_header_stru *)pst_eth + 1); /* 以太网帧格式：跳过 目的地址6 源地址6 帧类型2，到ip头 */
    iphdr_len = iph->ihl * 4; /* 获取ip头总字节：ihl * 4 */
    arp = (struct arphdr *)((oal_uint8 *)iph + iphdr_len); /* 从ip头偏移到arp数据体首地址 */
    ...
    dmac_msdu_stru *pst_msdu)
{
    ...
    if ((pst_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
        (mac_vap_find_user_by_macaddr(&pst_vap->st_vap_base_info, pst_msdu->auc_da, &us_user_dix) == OAL_SUCC)) {
        /* 将MSDU转化为以太网格式的帧 */
...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
            if ((pst_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
                (mac_vap_find_user_by_macaddr(&pst_vap->st_vap_base_info, st_msdu.auc_da, &us_user_dix) == OAL_SUCC)) {
                /* 将MSDU转化为以太网格式的帧 */
                hmac_rx_frame_80211_to_eth(st_msdu.pst_netbuf, st_msdu.auc_da, st_msdu.auc_sa);
                /* 将MSDU加入到netbuf链的最后 */
