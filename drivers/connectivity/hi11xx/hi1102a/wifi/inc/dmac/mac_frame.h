...
#define MAC_IS_WAVETEST_STA(pauc_bssid) (\
        (pauc_bssid[0] == 0x00)\
        &&  ((pauc_bssid[1] == 0x01) || (pauc_bssid[1] == 0x02))\
        &&  (pauc_bssid[2] == 0x01)\
)
/* ICMP协议报文 */
...
#endif
extern oal_uint32  mac_rx_report_80211_frame(oal_uint8 *pst_mac_vap, oal_uint8 *pst_rx_cb,
...
{
    memcpy_s(puc_addr, WLAN_MAC_ADDR_LEN, puc_mac_hdr + 4, WLAN_MAC_ADDR_LEN); /* 拷贝地址1(跳过head 4字节) */
}


OAL_STATIC OAL_INLINE oal_void mac_get_address2  (oal_uint8 *puc_mac_hdr, oal_uint8 *puc_addr)
{
    memcpy_s(puc_addr, WLAN_MAC_ADDR_LEN, puc_mac_hdr + 10, WLAN_MAC_ADDR_LEN); /* 拷贝地址2(跳过head与地址1 10字节) */
}

OAL_STATIC OAL_INLINE oal_void  mac_get_address3(oal_uint8 *puc_mac_hdr, oal_uint8 *puc_addr)
{
    memcpy_s(puc_addr, WLAN_MAC_ADDR_LEN, puc_mac_hdr + 16, WLAN_MAC_ADDR_LEN); /* 拷贝地址3(跳过head、地址1、2 16字节) */
}
...
OAL_STATIC OAL_INLINE oal_void  mac_get_qos_ctrl(oal_uint8 *puc_mac_hdr, oal_uint8 *puc_qos_ctrl)
{
    if (mac_is_4addr(puc_mac_hdr) != OAL_TRUE) {
        memcpy_s(puc_qos_ctrl, MAC_QOS_CTL_LEN, puc_mac_hdr + MAC_QOS_CTRL_FIELD_OFFSET, MAC_QOS_CTL_LEN);
        return;
    }
...
extern oal_void    mac_set_rrm_enabled_cap_field_cb(oal_void *pst_vap, oal_uint8 *puc_buffer,
                                                    oal_uint8 *puc_ie_len);
...
#endif
extern oal_void mac_set_ext_capabilities_ie_cb(oal_void *pst_mac_vap, oal_uint8 *puc_buffer,
                                               oal_uint8 *puc_ie_len);
...
extern oal_rom_cb_result_enum_uint8 mac_set_rsn_ie_cb(oal_void *pst_vap, oal_uint8 *puc_pmkid,
                                                      oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len,
                                                      oal_uint32 *pul_ret);
extern oal_rom_cb_result_enum_uint8 mac_set_wpa_ie_cb(oal_void *pst_vap, oal_uint8 *puc_buffer,
                                                      oal_uint8 *puc_ie_len, oal_uint32 *pul_ret);
