...
static void get_dsi_dphy_ctrl(struct dpu_fb_data_type *dpufd, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	if (NULL == phy_ctrl || NULL == dpufd) {
		DPU_FB_ERR("dpufd or phyctrl is null.\n");
		return;
	}
	pinfo = &(dpufd->panel_info);
	...
	// clocked by TXBYTECLKHS
	data_t_hs_prepare = get_data_t_hs_prepare(dpufd, accuracy, ui);
...
static void get_dsi_cphy_ctrl(struct dpu_fb_data_type *dpufd, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	if (NULL == dpufd || NULL == phy_ctrl) {
		DPU_FB_ERR("dpufd or phy_ctrl is null.\n");
		return;
	}
	pinfo = &(dpufd->panel_info);
...
{
	struct dpu_panel_info *pinfo = NULL;
	pinfo = &(dpufd->panel_info);//lint !e838
...
/*lint +e715*/
static void mipi_config_cphy_spec1v0_parameter(char __iomem *mipi_dsi_base, struct dpu_panel_info *pinfo)
{
	uint32_t i;
	uint32_t addr = 0;
...
static void mipi_config_dphy_spec1v2_parameter(char __iomem *mipi_dsi_base, struct dpu_panel_info *pinfo)
{
	uint32_t i;
	uint32_t addr = 0;
...
int mipi_dsi_clk_enable(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->index == PRIMARY_PANEL_IDX) {
		...
		if (clk_tmp) {
			...
			if (ret) {
				DPU_FB_ERR("fb%d dss_dphy0_ref_clk clk_prepare failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
			...
			if (ret) {
				DPU_FB_ERR("fb%d dss_dphy0_ref_clk clk_enable failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
		}
		...
		if (clk_tmp) {
			ret = clk_prepare(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_dphy0_cfg_clk clk_prepare failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
			...
			ret = clk_enable(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_dphy0_cfg_clk clk_enable failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
		}
		...
		if (clk_tmp) {
			ret = clk_prepare(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_pclk_dsi0_clk clk_prepare failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
			...
			ret = clk_enable(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_pclk_dsi0_clk clk_enable failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
		}
	...
	if (is_dual_mipi_panel(dpufd) || (dpufd->index == EXTERNAL_PANEL_IDX)) {
		...
		if (clk_tmp) {
			ret = clk_prepare(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_dphy1_ref_clk clk_prepare failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
			...
			ret = clk_enable(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_dphy1_ref_clk clk_enable failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
		}
		...
		if (clk_tmp) {
			ret = clk_prepare(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_dphy1_cfg_clk clk_prepare failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
			...
			ret = clk_enable(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_dphy1_cfg_clk clk_enable failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
		}
		...
		if (clk_tmp) {
			ret = clk_prepare(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_pclk_dsi1_clk clk_prepare failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
			...
			ret = clk_enable(clk_tmp);
			if (ret) {
				DPU_FB_ERR("fb%d dss_pclk_dsi1_clk clk_enable failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
		}
	...
	char __iomem *mipi_dsi_base)
{
	...
	if (NULL == dpufd || NULL == mipi_dsi_base) {
		DPU_FB_ERR("dpufd or mipi_dsi_base is null.\n");
		return 0;
	}
...
REDO:
	if (redo_count > 100) {
		DPU_FB_ERR("mipi phy pll retry fail, phy is not ready.\n");
		return 0;
	}
	redo_count ++;
...
static int mipi_dsi_off_sub(struct dpu_fb_data_type *dpufd, char __iomem *mipi_dsi_base)
{
	...
	if(NULL == mipi_dsi_base){
		DPU_FB_ERR("mipi_dsi_base is NULL");
		return -EINVAL;
	}
	...
	char __iomem *mipi_dsi_base, struct mipi_dsi_phy_ctrl* phy_ctrl)
{
	...
	pixel_clk = mipi_pixel_clk(dpufd);
	if (pixel_clk == 0) {
		DPU_FB_ERR("The value of pixel_clk is 0\n");
		return;
	}
	/*lint -e737 -e776 -e712*/
	if (pinfo->mipi.phy_mode == DPHY_MODE) {
...
int mipi_dsi_bit_clk_upt_isr_handler(struct dpu_fb_data_type *dpufd)
{
	...
	if (pinfo->mipiclk_updt_support_new) {
		...
		vfp_time = inp32(dpufd->mipi_dsi0_base + MIPIDSI_VID_HLINE_TIME_OFFSET) & 0x7fff;
		if (lane_byte_clk != 0) {
			vfp_time = vfp_time * (dpufd->panel_info.ldi.v_front_porch + 10) / ((uint32_t)(lane_byte_clk/1000000));
		} else {
	...
	if (pinfo->mipiclk_updt_support_new) {
		while ((!is_ready) && (timediff < vfp_time)) {
			is_ready = check_pctrl_trstop_flag(dpufd, 10);
			dpufb_get_timestamp(&tv1);
			timediff = dpufb_timestamp_diff(&tv0, &tv1);
		}
		DPU_FB_INFO("timediff=%d us, vfp_time=%d us.\n", timediff, vfp_time);
	} else {
		is_ready = check_pctrl_trstop_flag(dpufd, 40);
	}
	...
	if (is_mipi_cmd_panel(dpufd)) {
		...
		if (pinfo->mipiclk_updt_support_new) {
			...
			if (is_dual_mipi_panel(dpufd)) {
				set_reg(dpufd->mipi_dsi1_base + MIPIDSI_LPCLK_CTRL_OFFSET, 0x0, 1, 0);
				mipi_dsi_set_cdphy_bit_clk_upt_video(dpufd, dpufd->mipi_dsi1_base, &phy_ctrl);
				set_reg(dpufd->mipi_dsi1_base + MIPIDSI_LPCLK_CTRL_OFFSET, 0x1, 1, 0);
			}
		} else {
			mipi_dsi_set_cdphy_bit_clk_upt_video(dpufd, dpufd->mipi_dsi0_base, &phy_ctrl);
			if (is_dual_mipi_panel(dpufd)) {
