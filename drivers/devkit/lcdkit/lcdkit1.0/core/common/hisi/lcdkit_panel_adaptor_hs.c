/*
 * lcdkit_panel_adaptor_hs.c
 *
 * lcdkit panel adaptor hisi function for lcd driver
 *
...
void lcdkit_updt_porch(struct platform_device* pdev, int scence)
{
	...
	case LCDKIT_FPS_SCENCE_IDLE:
		if (lcdkit_info.panel_infos.fps_30_porch_param.buf != NULL)
		{
			pinfo->ldi_updt.h_back_porch =
				lcdkit_info.panel_infos.fps_30_porch_param.buf[0];
			pinfo->ldi_updt.h_front_porch =
				lcdkit_info.panel_infos.fps_30_porch_param.buf[1];
			pinfo->ldi_updt.h_pulse_width =
				lcdkit_info.panel_infos.fps_30_porch_param.buf[2];
			pinfo->ldi_updt.v_back_porch =
				lcdkit_info.panel_infos.fps_30_porch_param.buf[3];
			pinfo->ldi_updt.v_front_porch =
				lcdkit_info.panel_infos.fps_30_porch_param.buf[4];
			pinfo->ldi_updt.v_pulse_width =
				lcdkit_info.panel_infos.fps_30_porch_param.buf[5];
	...
	case LCDKIT_FPS_SCENCE_EBOOK:
		if (lcdkit_info.panel_infos.fps_55_porch_param.buf != NULL)
		{
			pinfo->ldi_updt.h_back_porch =
				lcdkit_info.panel_infos.fps_55_porch_param.buf[0];
			pinfo->ldi_updt.h_front_porch =
				lcdkit_info.panel_infos.fps_55_porch_param.buf[1];
			pinfo->ldi_updt.h_pulse_width =
				lcdkit_info.panel_infos.fps_55_porch_param.buf[2];
			pinfo->ldi_updt.v_back_porch =
				lcdkit_info.panel_infos.fps_55_porch_param.buf[3];
			pinfo->ldi_updt.v_front_porch =
				lcdkit_info.panel_infos.fps_55_porch_param.buf[4];
			pinfo->ldi_updt.v_pulse_width =
				lcdkit_info.panel_infos.fps_55_porch_param.buf[5];
	...
	default:
		if (lcdkit_info.panel_infos.fps_60_porch_param.buf != NULL)
		{
			pinfo->ldi_updt.h_back_porch =
				lcdkit_info.panel_infos.fps_60_porch_param.buf[0];
			pinfo->ldi_updt.h_front_porch =
				lcdkit_info.panel_infos.fps_60_porch_param.buf[1];
			pinfo->ldi_updt.h_pulse_width =
				lcdkit_info.panel_infos.fps_60_porch_param.buf[2];
			pinfo->ldi_updt.v_back_porch =
				lcdkit_info.panel_infos.fps_60_porch_param.buf[3];
			pinfo->ldi_updt.v_front_porch =
				lcdkit_info.panel_infos.fps_60_porch_param.buf[4];
			pinfo->ldi_updt.v_pulse_width =
				lcdkit_info.panel_infos.fps_60_porch_param.buf[5];
...
{
	uint32_t rgbw_bl_level;
	uint32_t rgbw_backlight;
...
{
	int i;
	if (len < DDIC_PRODUCT_ID_OFFSET) {
		LCDKIT_ERR("len is less than DDIC_PRODUCT_ID_OFFSET\n");
		return -1;
	}
...
void lcdkit_update_lcd_brightness_info(void)
{
	...
	if(lcdkit_info.panel_infos.lcd_brightness_color_uniform_support)
	{
		if (lcdkit_info.panel_infos.lcd_color_oeminfo.tc_flag != 0)
