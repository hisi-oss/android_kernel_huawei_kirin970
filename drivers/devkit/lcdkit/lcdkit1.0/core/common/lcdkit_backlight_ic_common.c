...
int lcdkit_backlight_ic_set_brightness(unsigned int level)
{
	...
	*/
	if (!strcmp(chip_name, "hw_rt8555")) {
		...
		if ((level & 0xFF) == 0xFF)
			level -= 1;
	} else if (!strcmp(chip_name, "hw_mp3314")) {
		/*
	...
	if (plcdkit_bl_ic->bl_config.msb_before_lsb)
...
void lcdkit_parse_backlight_ic_config(struct device_node *np)
{
	...
	lcdkit_parse_backlight_ic_param(np, "lcdkit-bl-ic-msb-before-lsb", &g_bl_config.msb_before_lsb);
	lcdkit_parse_backlight_ic_param(np, "lcdkit-bl-ic-skip-init", &g_bl_config.bl_skip_init);
	...
	if (g_bl_config.msb_before_lsb) {
		lcdkit_backlight_value_swap(&g_bl_config.bl_lsb_reg_cmd.cmd_reg,
			&g_bl_config.bl_msb_reg_cmd.cmd_reg);
		LCDKIT_INFO("backlight ic swap bl_lsb_reg and bl_msb_reg\n");
	}
...
static unsigned int backlight_test_led_open(struct lcdkit_bl_ic_device *pblchip, int led)
{
	...
	ret = lcdkit_backlight_ic_write_byte(pblchip, pblchip->bl_config.bl_enable_cmd.cmd_reg,
		(~(1 << (uint32_t)(1 + led))) & pblchip->bl_config.bl_enable_cmd.cmd_val);
	if(ret < 0)
	{
		LCDKIT_ERR("TEST_ERROR_I2C\n");
		return TEST_ERROR_I2C;
...
static int lcdkit_backlight_ic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!plcdkit_bl_ic->bl_config.bl_skip_init) {
		ret = lcdkit_backlight_ic_inital();
		if(ret < 0)
		{
			devm_kfree(&client->dev, plcdkit_bl_ic);
