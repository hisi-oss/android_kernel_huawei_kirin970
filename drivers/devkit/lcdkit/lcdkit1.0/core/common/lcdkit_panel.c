...
static int lcdkit_gpio_detect_event(void)
{
	...
	if (!lcdkit_info.panel_infos.tp_esd_event) {
		LCDKIT_ERR("tp_esd_event not support!\n");
		return ret;
	}

	if (lcdkit_info.panel_infos.esd_gpio_detect_num) {
		ret = gpio_request(
			lcdkit_info.panel_infos.esd_gpio_detect_num,
			"esd-gpio-detect-num");
		if (ret) {
			LCDKIT_ERR("esd_detect_gpio[%d] request fail!\n",
				lcdkit_info.panel_infos.esd_gpio_detect_num);
			 return ret;
		}
		...
		if (ret) {
			LCDKIT_ERR("gpio_err_flag[%d] direction set fail!\n",
				lcdkit_info.panel_infos.esd_gpio_detect_num);
			goto gpio_detect_exit;
		}
	}
	for (i = 0; i < GPIO_CHECK_TIMES; i++) {
		gpio_detect_value = (u32)gpio_get_value(
			lcdkit_info.panel_infos.esd_gpio_detect_num);
		if (gpio_detect_value ==
			lcdkit_info.panel_infos.esd_gpio_normal_value) {
			break;
		}
		if (i == (GPIO_CHECK_TIMES - 1)) {
...
gpio_detect_exit:
	gpio_free(lcdkit_info.panel_infos.esd_gpio_detect_num);
	return ret;
}
#endif
...
#ifdef CONFIG_HUAWEI_THP
	if (lcdkit_info.panel_infos.esd_gpio_detect_support) {
		ret = lcdkit_gpio_detect_event();
		if (ret) {
			LCDKIT_ERR("lcdkit_gpio_detect_event fail\n");
			return ret;
		}
	}
#endif
...
static ssize_t lcdkit_set_display_region(void* pdata, void* dirty)
{
	...
	lcdkit_info.panel_infos.display_region_cmds.cmds[0].payload[4] = ((uint32_t)(dirty_region->x + dirty_region->w - 1)) & 0xff;
	...
	lcdkit_info.panel_infos.display_region_cmds.cmds[1].payload[4] = ((uint32_t)(dirty_region->y + dirty_region->h - 1)) & 0xff;
