...
#define WORK_DURATION_MAX                  3100
#define WORK_DURATION_MS_NORMAL_STD        3100
...
enum motor_dist_control_type {
	PULSE_COUNTER = 100,
	PULSE_GEN_DUR_TIMER
};

enum motor_drv_step_mode {
	STEP_MODE_GUARD_MIN,
 ...
 */
enum motor_start_stop_curve_style {
	NO_START_NO_STOP,
	SLOW_START_NO_STOP,
	SLOW_START_SLOW_STOP
};

enum motor_step_gen_manner {
	GPIO_STEP,
	PWM_STEP
};

enum motor_step_voltage_type {
	NORMAR_VOLTAGE,
	HIGI_VOLTAGE,
	RESERVE_VOLTAGE,
...
struct motor_drv_data {
	...
	struct wakeup_source motor_wake_lock;
	...
	int cur_work_dur_ms;
	int cur_step_value;
	enum motor_speed_mode speed_mode;
	enum motor_direction cur_direction;
	...
	int cur_speed;
	enum motor_drv_state cur_state;
	enum motor_start_stop_curve_style curve_style;
	int motor_walk_distance_tenth;
	int motor_walk_time_ms;
	enum motor_dist_control_type dist_control_type;
	...
	enum motor_step_gen_manner step_gen_manner;
...
static const struct of_device_id g_aw8646_match[] = {
	{.compatible = "huawei,stepper_motor"},
	{},
};
enum motor_drv_step_mode g_step_motor_work_mode;
...
static void speed_start_control(struct motor_drv_data *di, bool speed_control)
{
	s64 secs;
	unsigned long nsecs;
	if (!di) {
...
#ifdef CONFIG_PWM
static void pwm_step_control(struct motor_drv_data *di, bool enable)
{
 ...
 */
static const char *power_down_reason(enum motor_drv_state pd_state)
{
	switch (pd_state) {
 ...
 */
static int drv_request_gpio(struct motor_drv_data *di, const char *name,
				int *gpio_out)
{
...
/*
 * Enable aw8646's power.
 *
 * @param di the device info struct.
 * @return void.
 */
static void aw8646_power(int gpio_vm)
{
...
/*
 * Schedule one stop work for stopping aw8646, used in irq context.
 *
 * @param di the device info struct.
 * @return void.
 */
static void aw8646_stop(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
 ...
 * @return void.
 */
static void aw8646_calc_full_speed_run_time(struct motor_drv_data *di,
...
static void aw8646_calc_up_down_runtime(struct motor_drv_data *di, int start_steps_count)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
 ...
 */
static int aw8646_param_init_calc(struct motor_drv_data *di)
{
	int start_average_speed;
	int start_steps_count = 0;
	int pulse_count_per_cycle;
	...
	aw8646_calc_up_down_runtime(di, start_steps_count);
	aw8646_calc_full_speed_run_time(di, pulse_count_per_cycle);
	return SUCCESS;
}
 ...
 * @return int SUCCESS or FAILURE.
 */
static int aw8646_param_init(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return FAILURE;
	...
	if (aw8646_param_init_calc(di) == FAILURE)
		goto error_end;
 ...
 * @return void.
 */
static void aw8646_speed_control_handle_speed_up(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
 ...
 */
static enum hrtimer_restart drv_speed_ctrl_handle_down(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
...
{
	int old_speed;
	struct motor_drv_data *di = container_of(timer,
		struct motor_drv_data, speed_control);
	...
	if (di->cur_state == STATE_SPEED_UP) {
		aw8646_speed_control_handle_speed_up(di);
 ...
 * @return hrtimer_restart whether the timer still needs restart.
 */
static enum hrtimer_restart aw8646_step_timer_func(struct hrtimer *timer)
{
	int current_speed;
	struct motor_drv_data *di = NULL;

	di = container_of(timer, struct motor_drv_data, step_timer);
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return HRTIMER_NORESTART;
	}
 ...
 * @return void.
 */
static void aw8646_step_start(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
 ...
 * @return void.
 */
static void aw8646_step_stop(const struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
...
/*
 * Set the rotation direction of the motor drived by aw8646.
 *
 * @param di the device info struct.
 * @param dir the needed direction.
 * @return void.
 */
static void aw8646_direction(int gpio_dir, int dir)
{
 ...
 * @return void.
 */
static void aw8646_set_dir_in(int gpio_dir, int dir)
{
	if (dir == 0)
		aw8646_direction(gpio_dir, GPIO_VAL_LOW);
	else
		aw8646_direction(gpio_dir, GPIO_VAL_HIGH);
}
 ...
 * @return void.
 */
static void aw8646_set_dir_out(int gpio_dir, int dir)
{
	if (dir == 0)
		aw8646_direction(gpio_dir, GPIO_VAL_HIGH);
	else
...
/*
 * Set the auto decay pin state of aw8646.
 *
 * @param di the device info struct.
 * @param val HIGH or LOW.
 * @return void.
 */
static void aw8646_adec(int gpio_decay, int val)
{
	...
	if (gpio_direction_output(gpio_decay, val))
		hwlog_err("set aw8646_decay:%d fail\n", val);
}

/*
 * Set the sleep pin state of aw8646.
 *
 * @param di the device info struct.
 * @param enable true to enter sleep state, false to get out of sleep state.
 * @return void.
 */
static void aw8646_sleep(int gpio_sleep, bool enable)
{
	if (enable) {
...
/*
 * Set the enable pin state of aw8646.
 *
 * @param di the device info struct.
 * @param enable true to enable false to disable.
 * @return void.
 */
static void gpio_enable(int gpio_enable, bool enable)
{
	if (enable) {
...
static void aw8646_enable(const struct motor_drv_data *di, bool enable)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
	}
	gpio_enable(di->gpio_enable, enable);
}

/*
 * Enable the fault irq of aw8646.
 *
 * @param di the device info struct.
 * @return void.
 */
static void aw8646_enable_fault_irq(int irq_fault)
{
	enable_irq(irq_fault);
}

/*
 * Disable the fault irq of aw8646.
 *
 * @param di the device info struct.
 * @return void.
 */
static void aw8646_disable_fault_irq(int irq_fault)
{
	disable_irq_nosync(irq_fault);
}

/*
 * Set the pins of aw8646 to idle state in case waste of power.
 *
 * @param di the device info struct.
 * @return void.
 */
static void aw8646_reset_pins_state(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
...
static void aw8646_poweroff(int gpio_vm)
{
	if (gpio_direction_output(gpio_vm, GPIO_VAL_LOW))
		hwlog_err("turn off power failed\n");
}

/*
 * Stop the aw8646, used in process context.
 *
 * @param di the device info struct.
 * @return void.
 */
static void aw8646_stop_immediately(struct motor_drv_data *di)
{
	enum motor_drv_state cur_state;
	...
	aw8646_disable_fault_irq(di->irq_fault);

	aw8646_step_stop(di);
	aw8646_enable(di, false);
	aw8646_sleep(di->gpio_sleep, true);
	aw8646_poweroff(di->gpio_vm);

	aw8646_reset_pins_state(di);
...
/*
 * The work function for stopping aw8646.
 *
 * @param work the work_struct for stopping aw8646.
 * @return void.
 */
static void aw8646_stop_work_func(struct work_struct *work)
{
	struct motor_drv_data *di = container_of(work, struct motor_drv_data, stop_work);

	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
	}
	aw8646_stop_immediately(di);
}

static enum hrtimer_restart aw8646_protect_timer_func(struct hrtimer *timer)
{
	struct motor_drv_data *di = container_of(timer, struct motor_drv_data,
		overtime_timer);
...
/*
 * Handler for aw8646 enable irq timer.
 *
...
static enum hrtimer_restart drv_irq_enable_timer_func(struct hrtimer *timer)
{
	...
	aw8646_enable_fault_irq(di->irq_fault);
	return HRTIMER_NORESTART;
}

/*
 * Start the aw8646, used in process context.
 *
 * @param di the device info struct.
 * @return int SUCCESS or FAILURE.
 */
static void set_mode(struct motor_drv_data *di, enum motor_drv_step_mode step_mode)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
	...
	case FULL_STEP_RISING_ONLY:
		...
		gpio_direction_output(di->gpio_mode0, GPIO_VAL_LOW);
		break;
	case THIRTY_TWO_STEP_RISING_ONLY:
		gpio_direction_input(di->gpio_mode0);
		gpio_direction_output(di->gpio_mode1, GPIO_VAL_HIGH);
		break;
	default:
		hwlog_err("set_mode unknown step_mode:%d\n", step_mode);
		break;
	}
}

static void start_motor(const struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
	...
	aw8646_sleep(di->gpio_sleep, true);
	aw8646_enable(di, false);
	aw8646_power(di->gpio_vm);
	set_mode(di, g_step_motor_work_mode);

	mdelay(di->power_on_delay_ms);
	aw8646_adec(di->gpio_decay, GPIO_VAL_HIGH);
	if (di->cur_direction == DIR_ROTATE_IN)
		aw8646_set_dir_in(di->gpio_dir, di->direction);
	else if (di->cur_direction == DIR_ROTATE_OUT)
		aw8646_set_dir_out(di->gpio_dir, di->direction);
	else
		hwlog_err("unknown direction\n");

	aw8646_sleep(di->gpio_sleep, false);
	mdelay(di->sleep_wake_delay_ms);
	aw8646_enable(di, true);
	aw8646_step_start(di);
}

static int aw8646_start_immediately(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return FAILURE;
	...
	if (aw8646_param_init(di) != SUCCESS) {
	...
	hrtimer_start(&di->overtime_timer,
		...
		HRTIMER_MODE_REL);
	print_ts("aw8646_start");

	mutex_unlock(&di->start_stop_mutex);
	return SUCCESS;
}

static void aw8646_wake_lock(struct motor_drv_data *di)
{
	if (di->motor_wake_lock.active) {
		hwlog_info("[wake_lock] already locked\n");
		return;
	}

	__pm_stay_awake(&di->motor_wake_lock);
	hwlog_info("aw8646 wake_lock\n");
}

static void aw8646_wake_unlock(struct motor_drv_data *di)
{
	if (!di->motor_wake_lock.active) {
	...
	__pm_relax(&di->motor_wake_lock);
	hwlog_info("aw8646 wake_unlock\n");
}
 ...
 */
static int aw8646_push_pull(struct motor_drv_data *di, enum motor_speed_mode mode,
	enum motor_direction dir)
{
	...
	spin_unlock(&di->state_change_lock);
	ret = aw8646_start_immediately(di);
	mutex_unlock(&di->cmd_mutex);
 ...
 * @return -ERESTARTSYS if it was interrupted by a signal, SUCCESS otherwise.
 */
static int aw8646_push_out(struct motor_drv_data *di, enum motor_speed_mode mode)
{
	int ret;
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
	...
	lpcpu_idle_sleep_vote(ID_MOTOR, 1);
	ret = aw8646_push_pull(di, mode, DIR_ROTATE_OUT);
	lpcpu_idle_sleep_vote(ID_MOTOR, 0);
	return ret;
}
 ...
 * @return -ERESTARTSYS if it was interrupted by a signal, SUCCESS otherwise.
 */
static int aw8646_pull_back(struct motor_drv_data *di, enum motor_speed_mode mode)
{
	int ret;
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
	...
	lpcpu_idle_sleep_vote(ID_MOTOR, 1);
	ret = aw8646_push_pull(di, mode, DIR_ROTATE_IN);
	lpcpu_idle_sleep_vote(ID_MOTOR, 0);
	return ret;
}
 ...
 * @return void.
 */
static void aw8646_stop_emergency(struct motor_drv_data *di, enum motor_direction cur_dir)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
 ...
 * @return void.
 */
static void aw8646_stop_push_out(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
	}
	aw8646_stop_emergency(di, DIR_ROTATE_OUT);
}
 ...
 * @return void.
 */
static void aw8646_stop_pull_back(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
	}
	aw8646_stop_emergency(di, DIR_ROTATE_IN);
}

/*
 * Handler for fault irq, shutdown the aw8646 immediately,
 * running in process context.
 *
 * @param work the work_struct in work queue.
 * @return void.
 */
static void aw8646_fault_int_work_func(struct work_struct *work)
{
	struct motor_drv_data *di = container_of(work, struct motor_drv_data,
		fault_int_work);
...
/*
 * Handler for fault irq, disable it and shutdown the aw8646 immediately,
 * running in irq context.
 *
 * @data the data private to this irq.
 * @return irqreturn_t IRQ_HANDLED this irq is handled.
 */
static irqreturn_t aw8646_fault_irq_handle(int irq, void *data)
{
	struct motor_drv_data *di = data;
...
/*
 * Common entrance for controlling aw8646 for user space command.
 *
 * @param di the device info struct.
 * @val the command from user space.
 * @return int the command processed result -ERESTARTSYS or SUCCESS.
 */
static int aw8646_control(struct motor_drv_data *di, int val)
{
	enum motor_control_cmd cmd = (enum motor_control_cmd)val;
	...
	switch (cmd) {
	case CMD_PUSH_OUT:
		return aw8646_push_out(di, SPEED_NORMAL);
	case CMD_PULL_BACK_NORMAL:
		return aw8646_pull_back(di, SPEED_NORMAL);
	case CMD_PULL_BACK_FAST:
		return aw8646_pull_back(di, SPEED_FAST);
	case CMD_STOP_PUSH_OUT:
		aw8646_stop_push_out(di);
		break;
	case CMD_STOP_PULL_BACK:
		aw8646_stop_pull_back(di);
		break;
	default:
 ...
 * @return void.
 */
static void aw8646_get_pinctrl_res(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
 ...
 */
static bool aw8646_is_param_name_valid(const char *name, int len)
{
	int i;
 ...
 */
static int aw8646_get_param_name(const char *buf, size_t count,
	char *name, size_t max_len)
{
	...
	if (name_len > max_len)
		return FAILURE;
	if (strncpy_s(name, max_len, buf + start_pos, name_len) != EOK)
		hwlog_err("strncpy_s failed\n");
	if (aw8646_is_param_name_valid(name, name_len))
		return SUCCESS;
	else
		return FAILURE;
}
 ...
 * @return bool true if valid, false otherwise.
 */
static bool aw8646_is_param_value_valid(const char *value, int len)
{
	int i;
 ...
 * @return int SUCCESS of FAILURE.
 */
static int aw8646_get_param_value(const char *buf, size_t count,
	char *value, size_t max_len)
{
	...
	if (value_len > max_len)
		return FAILURE;
	if (strncpy_s(value, max_len, buf + start_pos, value_len) != EOK)
		hwlog_err("strncpy_s failed\n");
	if (aw8646_is_param_value_valid(value, value_len))
		return SUCCESS;
	else
		return FAILURE;
}
...
/*
 * Translate the step_mode to aw8646's related params.
 *
 * @param di the device info struct.
 * @param step_mode the enum val.
 * @return void.
 */
static void aw8646_param_set_step_mode(struct motor_drv_data *di,
	enum motor_drv_step_mode step_mode)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
 ...
 */
static bool aw8646_handle_gpio_set(const struct motor_drv_data *di,
	const char *name, int val)
{
	...
	if (strcmp("gpio_enable", name) == 0) {
		aw8646_enable(di, value == GPIO_VAL_LOW);
	} else if (strcmp("gpio_sleep", name) == 0) {
		aw8646_sleep(di->gpio_sleep, value == GPIO_VAL_LOW);
		...
		aw8646_adec(di->gpio_decay, value);
	} else if (strcmp("gpio_dir", name) == 0) {
		aw8646_direction(di->gpio_dir, value);
 ...
 * @return bool true for handled or false otherwise.
 */
static bool aw8646_handle_delay_param_set(struct motor_drv_data *di,
	const char *name, int val)
{
	bool handled = true;
 ...
 * @return void.
 */
static void aw8646_set_val_per_step_mode(const struct motor_drv_data *di, int val,
	int *var)
{
	if (!di || !var || val <= 0) {
		hwlog_err("%s param is invalid\n", __func__);
 ...
 * @return bool true for handled or false otherwise.
 */
static bool aw8646_handle_speed_param_start_stop_curve(struct motor_drv_data *di,
	const char *name, int val)
{
	bool handled = true;
 ...
 * @return void.
 */
static void aw8646_reset_target_speed(struct motor_drv_data *di,
	enum motor_drv_step_mode step_mode)
{
	int old_microsteps;
	...
	aw8646_param_set_step_mode(di, step_mode);
 ...
 * @return bool true for handled or false otherwise.
 */
static bool aw8646_handle_speed_param_set(struct motor_drv_data *di,
	const char *name, int val)
{
	bool handled = true;
 ...
 * @return bool true for handled or false otherwise.
 */
static bool aw8646_handle_overtime_protect_param_set(struct motor_drv_data *di,
	const char *name, int val)
{
	bool handled = true;
	if (!di || !name || val < 0) {
...
static int aw8646_param_debug(const struct motor_drv_data *di,
	const char *name, int val, size_t count)
{
	if (!di || !name || val < 0) {
		hwlog_err("%s param is invalid\n", __func__);
		return false;
	...
	if (aw8646_handle_gpio_set(di, name, val))
		return count;
	else  if (aw8646_handle_delay_param_set(di, name, val))
		return count;
	else if (aw8646_handle_speed_param_set(di, name, val))
		return count;
	else if (aw8646_handle_overtime_protect_param_set(di, name, val))
		return count;
	hwlog_err("unknown param:%s\n", name);
}
#endif

static void step_mode_set_speed(struct motor_drv_data *di, int value, int *speed)
{
...
static void step_motor_set_speed_mode(struct motor_drv_data *di, int val)
{
	int slow_step_speed;
 ...
 */
static ssize_t aw8646_param_set_value(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = aw8646_get_param_name(buf, count, name, PARAM_NAME_LEN);
	if (ret != SUCCESS) {
		hwlog_err("invalid format name\n");
		return count;
	}
	ret = aw8646_get_param_value(buf, count, value, PARAM_VALUE_LEN);
	if (ret != SUCCESS) {
...
#ifdef STEP_MOTOR_DEBUG
	aw8646_param_debug(di, name, val, count);
#endif

	return count;
}

static DEVICE_ATTR(param, 0200, NULL, aw8646_param_set_value);
static DEVICE_ATTR(emergency, 0200, NULL, aw8646_param_set_value);

static struct attribute *g_aw8646_attributes[] = {
	&dev_attr_param.attr,
	&dev_attr_emergency.attr,
	NULL
};

static const struct attribute_group g_aw8646_attr_group = {
	.attrs = g_aw8646_attributes,
};

static void parse_dt_configs(struct motor_drv_data *di)
{
	struct device_node *np = NULL;
	u32 tmp = 0;
	if (!di) {
	...
	if (of_property_read_u32(np, "step_gen_manner", &tmp) == 0)
		di->step_gen_manner = (enum motor_step_gen_manner)tmp;
	else
	...
	if (of_property_read_u32(np, "set_step_mode", &tmp) == 0)
		g_step_motor_work_mode = (enum motor_drv_step_mode)tmp;
	else
		g_step_motor_work_mode = FULL_STEP_RISING_ONLY;
	hwlog_info("step_work_mode is :%d, tmp: %d\n", g_step_motor_work_mode, tmp);
	if (of_property_read_u32(np, "set_start_stop_curve_style", &tmp) == 0)
		di->curve_style = (enum motor_start_stop_curve_style)tmp;
	else
...
static void aw8646_get_motor_param_from_dts(struct motor_drv_data *di)
{
	struct device_node *np = NULL;
...
static void aw8646_param_set_default_val(struct motor_drv_data *di)
{
	...
	parse_dt_configs(di);
	aw8646_param_set_step_mode(di, g_step_motor_work_mode);
	...
	aw8646_get_motor_param_from_dts(di);
}

static int aw8646_gpio_init(struct motor_drv_data *di)
{
	int ret;
	struct device_node *np = NULL;
	...
	if (drv_request_gpio(di, "gpio_mode1", &di->gpio_mode1) != SUCCESS)
		return FAILURE;
	if (drv_request_gpio(di, "gpio_mode0", &di->gpio_mode0) != SUCCESS)
		return FAILURE;
	...
	aw8646_fault_irq_handle, IRQF_TRIGGER_LOW | IRQF_NO_SUSPEND, "aw8646",
		di);
	if (ret != 0) {
...
static void step_motor_set_direction(struct motor_drv_data *di)
{
	unsigned int temp = 0;
	struct device_node *np = NULL;
...
static void step_motor_get_vdd(struct motor_drv_data *di)
{
	int ret;
	u32 tmp = 0;
	struct device_node *np = NULL;
...
static void step_motor_remove_vdd(struct motor_drv_data *di)
{
	if (di == NULL) {
...
static int aw8646_probe(struct platform_device *pdev)
{
	...
	if (!of_match_node(g_aw8646_match, pdev->dev.of_node)) {
	...
	di->pdev = &pdev->dev;
	aw8646_get_pinctrl_res(di);
	if (aw8646_gpio_init(di) != SUCCESS)
		goto aw8646_init_fail;

	di->ic_vendor = TI_VENDOR;
	aw8646_param_set_default_val(di);
	...
	INIT_WORK(&di->stop_work, aw8646_stop_work_func);
	INIT_WORK(&di->fault_int_work, aw8646_fault_int_work_func);

	hrtimer_init(&di->step_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	di->step_timer.function = aw8646_step_timer_func;
	...
	di->overtime_timer.function = aw8646_protect_timer_func;
	hrtimer_init(&di->irq_enable_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	di->irq_enable_timer.function = drv_irq_enable_timer_func;

	wakeup_source_init(&di->motor_wake_lock, "motor_wakelock");
	platform_set_drvdata(pdev, di);

	if (sysfs_create_group(&di->pdev->kobj, &g_aw8646_attr_group) != 0)
		goto aw8646_sysfs_fail;
...
aw8646_sysfs_fail:
	...
	wakeup_source_trash(&di->motor_wake_lock);
aw8646_init_fail:
...
{
	struct motor_drv_data *di =
		(struct motor_drv_data *)platform_get_drvdata(pdev);
	...
	sysfs_remove_group(&di->pdev->kobj, &g_aw8646_attr_group);
	return 0;
}
...
static struct platform_driver g_aw8646_driver = {
	.probe = aw8646_probe,
	...
	.driver = {
		.name   = "aw8646",
		.owner  = THIS_MODULE,
		.of_match_table = of_match_ptr(g_aw8646_match),
	},
};

static int __init aw8646_init(void)
{
	return platform_driver_register(&g_aw8646_driver);
}

static void __exit aw8646_exit(void)
{
	platform_driver_unregister(&g_aw8646_driver);
}

module_init(aw8646_init);
module_exit(aw8646_exit);
